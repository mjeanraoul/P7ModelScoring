...
def main():
    ...
    app_test_domain, numero_client_test = load_data_test()
    app_domain, numero_client = load_data_app('app_domain')
    #print(numero_client[0])
    # j'ai 3 variables
    # 1 - id_client : pour les prédictions
    # 2 - numero_client_test pour le fichier test
    # 3 - numero_client : pour fournir l'accès à tous les clients

    # Verif client OK
    #print("Info app_train_domain ", app_train_domain.info(verbose=True, buf=None, max_cols=None, memory_usage=None, show_counts=True, null_counts=None))
    #print("Info app_test_domain ",
    #      app_test_domain.info(verbose=True, buf=None, max_cols=None, memory_usage=None, show_counts=True,
    #                            null_counts=None))
    #print('Taille app_domain', app_domain.shape)
    ...
    def page_client():
        ...
        if st.button('Consultation'):
            consult = app_domain[app_domain.index == chk_id]
            st.subheader("**Mes données:**")
            st.dataframe(consult.style.highlight_max(axis=0))

            infos_client = app_domain[app_domain.index == chk_id]
            ...
            data_age = load_age_population(app_domain)
